...
const filters = ['すべて', '軌跡', '御朱印', '旅サマリー'] as const;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: palette.washi }} edges={['top']}>
      ...
      <ScrollView
        ...
        style={{ maxHeight: 56 }}
        contentContainerStyle={{ paddingHorizontal: space.lg, gap: space.lg, alignItems: 'center' }}
      >
        {filters.map((f) => {
          const on = filter === f;
          return (
            <Pressable key={f} onPress={() => setFilter(f)}>
              <AppText variant="bodyStrong" tone={on ? 'shu' : 'inkFaint'}>
                {f}
              </AppText>
            </Pressable>
          );
      ...
      >
        <Row style={{ alignItems: 'flex-start', gap: space.md }}>
          <View style={{ flex: 1, gap: space.lg }}>
            {left.map((c) => (
              <GalleryEntry key={c.id} card={c} width={colW} palette={palette} />
            ))}
          </View>
          <View style={{ flex: 1, gap: space.lg }}>
            {right.map((c) => (
    ...
    <View>
      ...
      <Row style={{ justifyContent: 'space-between' }}>
        <Pressable onPress={() => setLiked((v) => !v)}>
          <Row style={{ gap: 4 }}>
            <Ionicons
              ...
              size={16}
              color={liked ? palette.shu : palette.inkFaint}
            />
        ...
        <Row style={{ gap: space.md }}>
          <Pressable>
            <Ionicons name="bookmark-outline" size={16} color={palette.inkFaint} />
          </Pressable>
          <Pressable onPress={() => router.push('/trip/t1')}>
            <Row style={{ gap: 3 }}>
              <Ionicons name="map-outline" size={15} color={palette.aiSoft} />
              <AppText variant="small" tone="aiSoft">ルート参考</AppText>
            </Row>
          </Pressable>
        </Row>
      </Row>
